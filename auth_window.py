...
# from book_window import ProductWindow
#
...
class AuthWindow(QWidget):
    ...
    def check_auth(self):
        ...
        try:
            ...
            result = cur.fetchone()
            if result:
                from book_window import BookWindow  # <-- локальный импорт
                self.main_win = BookWindow(role=result[0], user_name=result[1], user_login=login)
                self.main_win.show()
                self.close()
            else:
                QMessageBox.warning(self, "Ошибка", "Неверный логин или пароль")
    ...
    def enter_as_guest(self):
        from book_window import BookWindow  # <-- локальный импорт
        self.main_win = BookWindow(role="Гость", user_name="Гость", user_login=None)
        self.main_win.show()
        self.close()